...
parser.add_argument('-hidden-size', type=int, default=128, help='max length of sentence')
...
attn_textrnn = model.AttentionModel(args)
if args.snapshot:
    print('\nLoading model from {}...\n'.format(args.snapshot))
    attn_textrnn.load_state_dict(torch.load(args.snapshot))

if args.cuda:
    torch.cuda.set_device(args.device)
    attn_textrnn = attn_textrnn.cuda()
try:
    train.train(train_iter, dev_iter, attn_textrnn, args)
