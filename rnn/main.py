...
parser.add_argument('-early-stopping',type=int,default=100000,help='iteration numbers to stop without performance increasing')
...
text_rnn = model.TextRNN(args)
if args.snapshot:
    print('\nLoading model from {}...\n'.format(args.snapshot))
    text_rnn.load_state_dict(torch.load(args.snapshot))

if args.cuda:
    torch.cuda.set_device(args.device)
    text_rnn = text_rnn.cuda()
try:
    train.train(train_iter,dev_iter,text_rnn,args)
